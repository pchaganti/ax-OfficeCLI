...
compatibility: basic border+fill (with a VML Fallback), multi-paragraph rich
text, a nested table, 45-degree rotation + gradient, vertical text, rounded
rectangle + shadow, side-by-side metric cards, a borderless transparent box,
fixed-height overflow, and z-order stacking (behindDoc + translucent overlay).

SDK twin of textbox.sh (officecli CLI). Both produce an equivalent
textbox.docx. The CLI script drives `officecli add` (for the heading
paragraphs) and `officecli raw-set --part /document --xpath ... --action
insertbefore --xml ...` (for each textbox, injected before the body sectPr).
This twin drives the **officecli Python SDK** (`pip install officecli-sdk`):
one resident is started and every paragraph + raw-set is shipped over the
named pipe in a single `doc.batch(...)` round-trip. Each item is the same
`{"command", ...}` dict you'd put in an `officecli batch` list — `add`
paragraphs carry `parent`/`type`/`props`, and `raw-set` items carry
`part`/`xpath`/`action`/`xml` (the exact batch fields the resident dispatches).
...
# ---------------------------------------------------------------- textbox XML
# Each fragment is the verbatim <w:p>...</w:p> wrapper from textbox.sh.
...
with officecli.create(FILE, "--force") as doc:
    items = [
        ...
        # Scenario 1: Basic Textbox (border + background, with VML Fallback)
        ...
        # Scenario 4: Rotated Textbox (45 degrees + gradient)
